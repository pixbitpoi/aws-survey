...
| `libexec/container.sh`（一時キーだけを渡して調査コンテナのイメージで 1 コマンド）・`libexec/inventory.sh`（コンテナに貸す列挙。1 行 1 JSON）・`libexec/menu.sh`（矢印キーの選択）・`libexec/keys.sh`・`libexec/commands/ls.sh` / `ec2.sh`・`lambda.sh` の引数なし（対話モード）・`show_extras` の案内 | `.agents/rules/credentials.md` の「利用者に見せる入口はリソース名」の段落。`inventory.sh` を偽 `aws` で直接叩くのが `tests/test_inventory.py`、偽 `docker` と pty で `ls` / `ec2` / `lambda` を通すのが `tests/test_resources.py`。`container/` には置かない理由は `.agents/rules/container.md` |
