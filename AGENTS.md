...
| 調査中の進め方・報告のあとのユーザーとの進め方・報告に何を書かせるか | ホストでは持たない。調査エージェント（`container/instructions/survey-agents.md` と `container/method/` の 3 本: `報告.md`・`調べ方.md`・`経路.md`）が自分で持つ。触るときは `.agents/rules/container.md` |
...
| `container/ec2`（調査コンテナの `ec2` ラッパー、二層の SSM セッションの後片付け、`--selftest`）・フックの `ec2` / `ssh` の判定・`Dockerfile` の session-manager-plugin・`container/method/経路.md` の EC2 の節・`survey-status` の登録済みホスト表示 | `.agents/rules/container.md` に加えて `docs/design-ec2-ssh.md` の第 7 節（自己診断の項目と SSM セッションの終わり方・起動時の後片付けの実測）・第 8 節（コンテナ側の部品とフックの判定）。通る例・落ちる例とラッパーの引数の渡し方・後片付けの呼び出し順は `tests/test_guards.py`（偽の `ssh` と `aws`）、配置は `tests/test_launcher.py`。`method/経路.md` の EC2 の節はゲートウェイの動詞（第 5 節）と食い違わせない |
...
- 調査の共通指示は `container/instructions/survey-agents.md`、詳細は `method/` の 3 本（`報告.md`・`調べ方.md`・`経路.md`）。番号を付けず、同じ規則を 2 か所に書かない。
- 報告に求めるのは「何をしていて、どう組まれていて、どこが気になるか」（`method/報告.md`）。守りの規則を足すときは、役に立つ成果物を出す指示より多くしない。
