...
検証用の空アカウントで行う。関数・レイヤー・実行ロールの作成と片付けは `tests/live/lambda/`（`launch.sh` / `check.sh` / `cleanup.sh`。
使い方は `tests/live/README.md`）。このツールの機能ではないので、管理者権限のプロファイルで動かす。

1. 一時キーを発行し直し、`PackedPolicySize` を控える（第 1 段の Deny を含むキーになる）: `aws-survey credentials` → `aws-survey verify`
2. `tests/live/lambda/launch.sh` で作る。Python の関数は版 1 に alias `live`、`$LATEST` は別のコード。Node.js の関数は
   100 KiB 超のバンドルとソースマップ（元ソースはバンドルと同じ処理）と `node_modules`。レイヤーは依存だけ
3. 対象フォルダで `aws-survey lambda pull --all`。見ること: 取り出し専用の一時キーの自己確認（Lambda 以外は拒否）、画面に `https://` や
   `X-Amz-` が出ないこと。取り出した `code/` は `tests/live/lambda/check.sh <対象フォルダ>` が 32 項目で見る（マスク・名前での除外・
