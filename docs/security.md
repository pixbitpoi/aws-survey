...
ホストの `aws-survey ls` / `ec2` / `lambda`（一覧）と `ssh verify` は、この調査コンテナと同じイメージを `docker run --rm` で
1 コマンドだけ動かします。渡すのは読み取り専用の一時キーの ro マウントだけで、`out/` も元プロファイルも渡しません。
一時キーで読めるものは調査コンテナの中でも外でも同じで、ホストの強い権限がコンテナに流れる経路にはなりません。
一覧は画面に出すだけで、ファイルには書きません。

報告の構成図（C4）は、調査エージェントが書いた Structurizr DSL（`out/report/c4/workspace.dsl`）をホストの `aws-survey c4` が
PNG にします（`scan` と対話の終わりに自動で走ります）。使うのは Structurizr の公式イメージで、渡すのは `out/report/c4/` の
マウントだけ、ネットワークは切って（`--network none`）動かします。一時キーも元プロファイルも渡らず、DSL の中身が外に出る経路もありません。
...
- `aws` は単独のコマンドのみ。パイプ・連結・`$()`・`bash -c` 経由・改行（`\` による行継続を含む）は拒否します（保存は `> out/…/名前.json` の形だけ許可）
...
検査したコマンドは、通過・拒否のどちらも `out/.survey/env/aws-audit.log` に記録します。
...
## 5. Lambda のコードを読む経路（任意）

`aws-survey lambda pull` で取り出した関数に限り、調査コンテナで**デプロイされているコード**を読めます。取り出さなければ、この経路はありません。

**この経路はソースコードを読みます。** 第 1 節で `codecommit:GitPull` を Deny しているのは、リポジトリにはソースコード全体
（履歴・ブランチ・まだデプロイしていないもの）があるからです。この経路が読むのは、Lambda にデプロイされている成果物だけです。
`codecommit:GitPull` の Deny は変えていません。

```mermaid
flowchart LR
    P["ホストの aws-survey lambda pull<br/>取り出し専用の一時キー（15 分・メモリ上だけ）"] -->|GetFunction → 署名付き URL を curl| Z["ホストの一時ディレクトリの zip"]
    Z -->|ro| X["使い捨てのコンテナ<br/>ネットワークなし・資格情報なし<br/>展開・拒否パターン・マスク"]
    X --> D["対象フォルダの code/"]
    D -->|ro マウント| C["調査コンテナ"]
```

- **調査コンテナの一時キーでは、コードの URL を取れません。** 第 1 節の表のとおり `lambda:GetFunction` と `lambda:GetLayerVersion` を Deny しています（フックでも拒否します）
- 取り出しはホストで行います。調査用ロールを、**Lambda の 4 つ（`GetFunction` / `GetLayerVersion` / `ListFunctions` / `ListAliases`）だけ**を
  許すセッションポリシーで借りた、**15 分**の一時キーを使います。このキーはファイルに書かず、調査コンテナにも渡しません。
  発行の直後に、同じキーで Lambda 以外（VPC の一覧）が拒否されることを確かめ、読めてしまったら何も取り出しません
- `get-function` の応答には、環境変数の値とコードの署名付き URL が入ります。要る項目だけを取り、環境変数は名前だけを控えます。
  URL は画面にもファイルにも出しません
- 展開は、ネットワークも資格情報も無い使い捨てのコンテナで行います。zip の中身は信用せず、`..` を含むパス・シンボリックリンク・
  展開後の大きさの上限を見ます。秘密らしい名前のファイル（第 4 節の EC2 と同じ拒否パターン）は置かず、名前と理由だけを残します。
  ただしソースコードの拡張子のファイル（`secrets.py` など）は、読むこと自体が目的なので名前では除かず、マスクだけを掛けます
- マスクは第 4 節と同じ規則です。ソースコードでは、`password = "..."` のように右辺が文字列リテラルのときだけ伏せます
  （`os.environ[...]` から読んでいる、という事実は構成として読みたいため）
- 生の zip は、ホストの一時ディレクトリごと消します。調査コンテナに届くのは、絞り込んだあとのコードだけです
- 関数は起動しません。取り出したコードも実行しません（調査コンテナでは `node` / `python3` を実行できません）。AWS には何も置きません

後片付けは `aws-survey lambda remove <関数名>`（全部なら `--all`）です。ホストに取り出したコードを消します。

## 6. これで保証されないこと

上の 5 つは、事故と行き過ぎを防ぐための多層の対策です。悪意のある相手を想定した境界ではありません。
...
`raw/` と突き合わせて検証します（`container/method/report.md`「検証」）。別のセッションであることが
...
**Lambda のコードに直書きされた秘密は、マスクをすり抜けることがあります。** マスクは経験則です。変数名の無い定数、
base64 などでエンコードされた値、連結して組み立てた値は伏せられません。

**Lambda のコードは対象の資産です。** 取り出した時点で、調査エージェント（とその提供元）が読む範囲に入ります。
取り出す前に、対象の管理者と合意しておいてください。
...
重要な判断に使うときは、`out/.survey/env/aws-audit.log` と `out/.survey/raw/` を人が直接読んでください。
