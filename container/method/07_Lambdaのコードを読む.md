...
- コードを取り出すことは、この環境ではできません。`code/` に無い関数が必要になったら、ユーザーに次の実行を依頼してください
  （そのリージョンの全関数なら `--all`、依存ライブラリの中身も要るなら `--with-deps` を足します）
...
違っていれば、取り出したあとに更新されています。そのまま読むときは古いことを記録し、必要ならユーザーに取り直しを依頼してください。
...
6. **設定と突き合わせる**: 実行ロールの権限（`iam list-attached-role-policies` / `list-role-policies` / `get-role-policy`）、
   トリガー（`lambda list-event-source-mappings`）、呼んでいるリソースが実在するか
...
| 形 | 置かれているもの | どうするか |
| --- | --- | --- |
| バンドルされた JavaScript | 固められたファイルと、ソースマップがあれば `src/_sources/` | `_sources/` を読む。`bundles[].map` が `none` / `no-sourcesContent` なら、固められたファイルから `grep` でサービス名・環境変数名だけを拾う |
...
| 依存ライブラリ | 名前と版の一覧だけ | 中身が必要なら、ユーザーに `--with-deps` での取り直しを依頼する |
