...
`code/` に無い関数は、設定とトリガーから役割を推し、コードで確かめたいことを `report/ユーザー確認事項.md` の「中を見れば分かること」に
取り出しの依頼文つきで挙げます（`method/04`）。取り出されたら、対話の回でもユーザーが応答できない回でも、その回で読んで報告の本文
（その系統の節）に書き直します（`method/00`・`method/02`）。
