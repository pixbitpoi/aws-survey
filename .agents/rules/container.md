...
| 報告に何を書けば役に立つか（骨子・構成図の DSL の見本・完了の基準・検証） | `method/report.md` |
...
`survey-status` は経路（登録済みホスト・取り出してあるコード）に加えて、報告の C4 図の状態も出す（DSL の有無・PNG の枚数・`_render-error.txt`）。
PNG はホストが作る（`.agents/rules/credentials.md`「報告の C4 図」）が、コンテナ側の文書にはそう書かず、
「PNG はこの環境の外で作られ、この回の終わりに同じ場所に置かれる」という見える事実だけを書く。
...
外からは分からないこと → 付録: 網羅性 → 付録: リソース一覧」。本文をサービスのカテゴリ順にすると、権限・ネットワーク・稼働の事実が別々の節に散り、
...
同じ種類が多いとき（共有ホスティングの CloudFront 35・S3 68）に 3 節の表が「CloudFront distribution 35 件」の 1 行に畳まれ、
S3 と IAM ユーザーは代表数件の確認で全体を語った（2026-09-13 に実対象で発生。初回の `scan` で掘り下げ切るのが狙いなので、
対話の回に頼らない）。対策は `method/report.md` の 3 節「経路の表」（入口 1 つ 1 行）と 9 節「リソース一覧」（全件 1 行 1 リソース）、
`method/survey.md` 工程 4 の「代表確認で済ませないもの」（種類ごとに全件で取るもの）、完了の基準と検証の「畳んでいないか」。
構成図だけは同じ形が 10 を超えて並ぶときに「他 N」でまとめてよい（表と付録が全件を持つ）。
