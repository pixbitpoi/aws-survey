...
`--json` で受けて `scan_codex_text`（jq）が agent_message とエラーだけを文章に戻す。stderr は素通し（ログイン切れの判定は `SCAN_LOG` の grep）。ホストから渡す指示は「基礎調査を報告（`report/構成報告.md`・`report/ユーザー確認事項.md`）まで・ユーザーに聞かずに終える・
...
初期調査が済んでいれば（`setup.scanned` あり）、一時キーを発行し直す前に前回の日時を出して「やり直しますか？ (y/N)」と聞く（打ち直しただけで数十分の調査を
始めないため。Enter はやり直さない）。端末でなければ `read_line` が諦めて `--force` を案内し、`scan --force` なら聞かずに進む。やり直しても `out/` は消さない。
